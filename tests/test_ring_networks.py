...
def test_ringmolecule():
    rm = pt.RingMolecule('@oo.\n.oo@', rings={'o':pt.Waveguide()}, coupling={'oo':0.3, 'standard':0.5})

def test_ringmolecule_with_copy_rings():
    rm = pt.RingMolecule('@oo@', rings={'o':pt.Waveguide()}, coupling=0.2, copy_rings=True)
...
def test_ringmolecule_initialize(tenv):
    rm = pt.RingMolecule('@oo@', rings={'o':pt.Waveguide()}, coupling={'oo':0.3, 'standard':0.5})
    rm.initialize(tenv)

def test_ringmolecule_wrong_type():
    with pytest.raises(ValueError):
        rm = pt.RingMolecule('.@oo@.', rings={'.':pt.Waveguide()}, coupling=0.2, type='wrongtype')
